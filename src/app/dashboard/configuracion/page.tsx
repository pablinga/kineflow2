...
function toFormState(settings: WorkspaceSettings | null): FormState {
  ...
  return {
    ...
    minBookingNoticeHours: settings.minBookingNoticeHours.toString(),
    name: settings.name,
...
export default function WorkspaceSettingsPage() {
  ...
  async function handleSaveSettings(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const notice = toNullableNumber(form.minBookingNoticeHours);
      ...
      if (notice === null || !Number.isInteger(notice) || notice < 0) {
        throw new Error("La anticipación mínima debe ser 0 o más horas.");
      }
        ...
        <form className="mt-4 grid gap-4 lg:grid-cols-2" onSubmit={handleSaveSettings}>
          ...
          <section className="rounded-lg border border-ocean-100 bg-white p-5 shadow-card sm:p-6">
            ...
            <div className="mt-5 grid gap-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <label className="block">
                  <FieldLabel>Duración predeterminada</FieldLabel>
                  <input
                    className="mt-1 min-h-11 w-full rounded-lg border border-ocean-100 px-3 text-sm outline-none focus:border-ocean-400 disabled:bg-slate-50"
                    disabled={!canManage}
                    min="1"
                ...
                <label className="block">
                  <FieldLabel>Anticipación mínima</FieldLabel>
                  <input
                    className="mt-1 min-h-11 w-full rounded-lg border border-ocean-100 px-3 text-sm outline-none focus:border-ocean-400 disabled:bg-slate-50"
                    disabled={!canManage}
                    min="0"
                    onChange={(event) =>
                      setForm((current) => ({
                        ...current,
                        minBookingNoticeHours: event.target.value,
                      }))
                    }
                    required
                    step="1"
