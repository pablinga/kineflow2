...
begin
  ...
  if coalesce(target_profile.account_type, 'KINESIOLOGO') <> 'KINESIOLOGO' then
    return null;
  end if;

  target_email := lower(coalesce(target_profile.email, (
  ...
  values (
    'PERSONAL',
    target_user_id,
    coalesce(nullif(target_profile.full_name, ''), 'Mi espacio'),
    target_email,
