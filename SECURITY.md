...
No hay variables privadas requeridas por la aplicacion en este momento.
...
- Guardar `MERCADO_PAGO_ACCESS_TOKEN` y el secreto/verificador del webhook como variables privadas de Vercel, sin `NEXT_PUBLIC_`.
...
Actualmente la aplicacion no define rutas API en `src/app`; el acceso a datos se realiza desde Supabase con Row Level Security.
